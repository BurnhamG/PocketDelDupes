...
class PocketConsoleTest(unittest.TestCase):
    ...
    def create_large_dict(items):
        ...
        while i <= items:
            yield i, {
                "item_id": str(i),
                "resolved_id": str(i),
                "given_url": f"http://www.example{i}.com",
                "given_title": f"Example Title {i}",
                "favorite": "0",
                "status": "0",
                "time_added": "1461738821",
                "time_updated": "1522220665",
                "time_read": "0",
                "time_favorited": "0",
                "sort_id": i - 1,
                "resolved_title": f"Example Title {i}",
                "resolved_url": f"http://www.example{i}.com",
                "word_count": str(
                    int((i * (random.random() * 10 if i < 1000 else random.random())))
                ),
            }
            i += 1
    ...
    def setUp(self):
        ...
        self.example_bad_article_list = {
            "1": {
                "item_id": "1",
                "resolved_id": "1",
                "given_url": "http://www.example.com",
                "given_title": "Example Title",
                "favorite": "0",
                "status": "0",
                "time_added": "1461738821",
                "time_updated": "1522220665",
            ...
            "2": {
                "item_id": "2",
                "resolved_id": "2",
                "given_url": "https://www.example2.com",
                "given_title": "Another Example",
                "favorite": "0",
                "status": "0",
        ...
        self.example_articles = {
            "1": {
                ...
                "sort_id": 0,
                "resolved_title": "Example Title",
                "resolved_url": "http://www.example.com",
                "word_count": "1250",
            },
            "2": {
                "item_id": "2",
                ...
                "tags": {
                    "Example_tag": {"item_id": "2", "tag": "Example_tag"},
                    "Second_tag": {"item_id": "2", "tag": "Second_tag"},
                },
                "word_count": "10000",
            },
        }
    ...
    @patch(
        "PocketDelDupes.Pocket.get_access_token",
        spec=PocketDelDupes.Pocket.get_access_token,
    )
    @patch("PocketDelDupes.webbrowser.open", spec=PocketDelDupes.webbrowser.open)
    @patch(
        "PocketDelDupes.Pocket.get_auth_url", spec=PocketDelDupes.Pocket.get_auth_url
    )
    @patch(
        "PocketDelDupes.Pocket.get_request_token",
        spec=PocketDelDupes.Pocket.get_request_token,
    )
    def test_authenticate(
        self, mock_pocket_request, mock_pocket_auth, mock_browser, mock_access_token
    ):
        with patch("PocketDelDupes.input") as mock_input:
            ...
            mock_pocket_request.assert_called_with(
                test_con_key, redirect_uri="https://ddg.gg"
            )
            mock_pocket_auth.assert_called()
            mock_pocket_auth.assert_called_with(
                req_token, redirect_uri="https://ddg.gg"
            )
    ...
    def test_url_test(self, mock_input, mock_output):
        id_list = [
            ...
            + self.example_bad_article_list["1"]["item_id"],
            "https://getpocket.com/a/read/"
            + self.example_bad_article_list["2"]["item_id"],
        ]
    ...
    def test_filterurl(self):
        urls = [
            ...
            "www.example.com/test?utm=referrer",
            "www.example.com/test?mc=referrer",
    ...
    def test_clean_db(self):
        ...
        for i in test_dict.keys():
            self.assertEqual(
                test_dict[i]["resolved_url"], self.example_articles[i]["resolved_url"]
            )
            self.assertEqual(
                test_dict[i]["word_count"], self.example_articles[i]["word_count"]
            )
            self.assertEqual(
                test_dict[i]["resolved_title"],
                self.example_articles[i]["resolved_title"],
            )
            self.assertEqual(
                test_dict[i]["time_added"], self.example_articles[i]["time_added"]
            )

        self.assertEqual(test_dict["1"]["tags"], {})
        self.assertEqual(
            test_dict["2"]["tags"], list(self.example_articles["2"]["tags"].keys())
        )
    ...
    def test_del_dupes(self, mock_instance):
        ...
        with patch.multiple(
            "PocketDelDupes", **{"input": DEFAULT, "print": DEFAULT}
        ) as mocks:
            for input_val in [["n"], ["y"], ["x", "y", "y"], [""], ["w", "n"], [""]]:
                ...
                if "n" in input_val[::2] or "" in input_val[::2]:
                    ...
                    self.assertEqual(
                        mock_filtered_dict, self.example_articles, msg=f"{input_val}"
                    )
    ...
    def test_items_to_manipulate_text_file(self, mock_input):
        ...
        with patch("PocketDelDupes.open", mo, create=True):
            ...
            mock_input.return_value = (
                "www.example.com, www.example2.com, www.example3.com/test"
            )
            test_item_list = PocketDelDupes.items_to_manipulate()
            self.assertEqual(
                test_item_list,
    ...
    def test_sort_items(self):
        ...
        with patch("PocketDelDupes.input") as mock_input:
            ...
            for v in key_list:
                ...
                if v == "l":
                    example_articles_sorted = sorted(
                        self.example_articles.items(),
                ...
                if v == "l":
                    ...
                    example_articles_sorted = sorted(
                        self.example_articles.items(), key=lambda x: x[1][key_list[v]]
                    )
                self.assertEqual(
                    example_return,
                    example_articles_sorted,
                ...
                self.assertEqual(
                    ...
                    example_articles_sorted,
                    msg=(
                        f"The sorted lists are not equal when retrying "
    ...
    def test_print_items_info(self, mock_print):
        art_timestamp = datetime.datetime.fromtimestamp(
            int(self.example_articles["1"]["time_added"])
        )
        PocketDelDupes.print_items_info(self.example_articles, "1", v_url="y")
        mock_print.assert_called_with(
            f"{self.example_articles['1']['resolved_title']}, added {art_timestamp}, with "
        ...
        art_timestamp = datetime.datetime.fromtimestamp(
            int(self.example_articles["2"]["time_added"])
        )
        PocketDelDupes.print_items_info(self.example_articles, "2", v_url="n")
        mock_print.assert_called_with(
            f"{self.example_articles['2']['resolved_title']}, added {art_timestamp}, with "
    ...
    def test_add_items(self, mock_instance, mock_items):
        ...
        PocketDelDupes.add_items(mock_instance)
        mock_instance.bulk_add.assert_called_with(
            url="http://" + mock_items.return_value[0]
        )
        mock_instance.commit.assert_called()
    ...
    def test_delete_items_url(self, mock_instance, mock_items):
        ...
        for item in self.example_articles:
            if (
                self.example_articles[item]["resolved_url"]
                == "http://" + mock_items.return_value[0]
            ):
                mock_id = item
    ...
    def test_get_article_url(self):
        returned_article = PocketDelDupes.get_article_url(
            self.example_articles, "http://www.example.com"
        )
        self.assertEqual(returned_article, "1")

    def test_get_article_url_bad(self):
        returned_article = PocketDelDupes.get_article_url(
            self.example_articles, "http://www.example.co"
        )
    ...
    def test_view_items_name(self, mock_display):
        with patch("PocketDelDupes.input") as mock_input:
            ...
            resolved_title_sorted_dict = dict(
                sorted(
                    self.example_articles.items(),
                    key=lambda x: x[1]["resolved_title"],
                    reverse=False,
                )
            )
    ...
    def test_tags_editing(self, mock_input, mock_print, mock_instance):
        ...
        mock_print.assert_called_with(
            "Here are the tags, along with their frequency: ",
        ...
        mock_input.side_effect = ["y"]
        PocketDelDupes.tags_editing(
            mock_instance, self.example_articles_cleaned_no_tags
        )
        mock_instance.tags_clear.assert_not_called()
        mock_instance.commit.assert_not_called()
    ...
    def test_save_articles_to_disk(self, mock_pickle, mock_os):
        ...
        with patch("PocketDelDupes.open", mo):
            ...
            PocketDelDupes.save_articles_to_disk(
                self.example_articles, str(int(time.time()))
            )
    ...
    def test_check_sync_date(self, mock_input):
        old_list = (
            "The saved list of articles has not been synchronized in two weeks. "
            + "Would you like to update the saved list? (Y/N) "
        )
        new_list = (
            "The saved list of articles has been synchronized in the past two weeks. "
            + "Would you like to update the saved list anyway? (Y/N) "
        )
        resync_msg = (
            "By default, we will show you all saved articles. Do you want to update ALL articles?\n"
        ...
        def set_more_list(length):
            return (
                f"You are requesting more articles than the {length} that are currently saved."
                " Would you like to update the saved list? (Y/N) "
            )
        ...
        list_length = 10
        return_val = PocketDelDupes.check_sync_date(
            test_date.timestamp(),
            length_of_current_list=list_length,
            ret_val=5,
            is_offline=False,
        )
        ...
        test_date = datetime.datetime.now() - datetime.timedelta(days=15)
        with self.assertRaises(SystemExit):
            PocketDelDupes.check_sync_date(
                test_date.timestamp(),
                length_of_current_list=list_length,
                ret_val=5,
                is_offline=False,
    ...
    def test_prepare_articles_dict(self, mock_url_test, mock_clean_db):
        ...
        ret_val = PocketDelDupes.prepare_articles_dict(
            ({"list": self.example_articles, "since": sync_time}, {"header": "HTTP"})
        )
    ...
    def test_retrieve_articles(
        self, mock_instance, mock_load, mock_start, mock_quantity
    ):
        sync_time = str(int(time.time()))
        ...
        def set_instance_response(value="all"):
                    ...
                    (
                        {
                            "list": {
                                z: k
                                for i, (z, k) in enumerate(
                                    self.large_article_list.items()
                                )
                                if (value * x)
                                <= i
                                < (value * x * 2 if x > 0 else value)
                            },
                            "since": sync_time,
                        },
                        {"header": "HTTP"},
            ...
            if divide_mod == 0:
                responses.append(({"list": {}, "since": sync_time}, {"header": "HTTP"}))
        ...
        with patch("PocketDelDupes.input") as mock_input:
            for num in [
                ...
                if num == "all":
                    ...
                    expected_result = {
                        k: v for k, v in clean_arts.items() if int(k) <= num
                    }

                self.assertEqual(
                    result,
                    (sync_time, expected_result),
                    msg=f"Not equal when num is {num}.",
                )
    ...
    def test_main(self, mock_instance):
        in_mocks = {
            "create_arg_parser": DEFAULT,
            "pocket_authenticate": DEFAULT,
            "retrieve_articles": DEFAULT,
            "del_dupes": DEFAULT,
            "save_articles_to_disk": DEFAULT,
            "add_items": DEFAULT,
            "delete_items": DEFAULT,
            "view_items": DEFAULT,
            "tags_editing": DEFAULT,
        ...
        with patch.multiple("PocketDelDupes", **in_mocks) as mocks:
            ...
            def assertion_tests(side_effects):
                mocks[
                    "create_arg_parser"
                ].return_value = PocketDelDupes.create_arg_parser().parse_args(["key"])
                mocks["create_arg_parser"].reset_mock()
                mocks["pocket_authenticate"].return_value = mock_instance
                mocks["retrieve_articles"].return_value = (
                    str(int(time.time())),
                    self.example_articles,
                )
                ...
                if side_effects[0] == "y":
                    mocks["del_dupes"].assert_called_with(
                        self.example_articles, mock_instance
                    )
