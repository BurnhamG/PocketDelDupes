...
def pocket_authenticate(con_key):
    request_token = Pocket.get_request_token(
        consumer_key=con_key, redirect_uri="https://ddg.gg"
    )
    auth_url = Pocket.get_auth_url(code=request_token, redirect_uri="https://ddg.gg")
...
def url_test(art_list):
    ...
    if var_error == 1:
        ...
        while not option:
            print(
                "Would you like the bad URLs [P]rinted on screen, [S]aved to a file, [N]either, or [B]oth?"
            )
        ...
        while not to_continue:
            to_continue = input(
                "Would you like to continue the program? [Y]es/[N]o"
            ).lower()
            if to_continue == "y":
                return
            elif to_continue == "n":
...
def del_dupes(masterdict, instance):
    ...
    dupe_dict = {
        masterdict[v[0]]["resolved_title"]: sorted(v, reverse=True)
        for k, v in reversed_dict.items()
        if len(reversed_dict[k]) > 1
    }
    ...
    if len(dupe_dict) > 0:
        ...
        while not confirm_delete:
            confirm_delete = input(
                ...
                "The article(s) most recently added will be removed. (Y/N)"
            ).lower()
            if confirm_delete == "":
                confirm_delete = "n"
...
def try_again():
    ...
    while decision == "":
        decision = input(
            "That is not valid, would you like to try again? (Y/N)"
        ).lower()
        if decision == "n":
            return False
        elif decision == "y":
...
def sort_items(dict_of_articles):
    ...
    while True:
        sort_order = input(
            "What would you like to sort by " "([N]ame/[D]ate/[L]ength/[U]RL)? "
        ).lower()
        if sort_order not in key_list.keys():
            ...
            while not direction:
                direction = input(
                    "How would you like to sort the articles "
                    "([F]orward/[B]ackward)? )"
                ).lower()
                if direction == "b":
                    reverse_opt = True
            ...
            if sort_order == "l":
                return sorted(
                    dict_of_articles.items(),
                    key=lambda x: int(x[1][key_list[sort_order]]),
                    reverse=reverse_opt,
                )
            else:
                return sorted(
                    dict_of_articles.items(),
                    key=lambda x: x[1][key_list[sort_order]],
                    reverse=reverse_opt,
                )
...
def print_items_info(all_articles, article, v_url="n"):
    ...
    time_art_added = datetime.datetime.fromtimestamp(
        int(all_articles[article]["time_added"])
    )
    if v_url == "y":
        ...
        output_end = ""
    print(
        f"{all_articles[article]['resolved_title']}, added {time_art_added}, with "
...
def display_items(articles_in_account):
    ...
    while not v_url:
        v_url = input(
            "Do you wish to view the URL with the article "
            "information (Y/N, default N)?"
        ).lower()
        if v_url == "":
            v_url = "n"
    ...
    while not art_disp:
        art_disp = input(
            "How many articles would you like to view "
...
def get_article_url(id_url_dict, url):
    for article in id_url_dict:
        if id_url_dict[article]["resolved_url"] == url:
            ...
            print(
                str(url) + " was not found in the list. "
        ...
        if not add_list:
            ...
            for item in add_list:
                ...
                if art_url:
                    ...
                    print(f"{item} is not a valid URL, " "and will be disregarded.")
            if valid_count > 0:
                instance.commit()
            ...
            for item in delete_list:
                ...
                if url:
                    ...
                    if re.search(r"[\D]", item):
                        print(
                            f"{item} is not a valid ID, please limit item IDs to numbers only."
                        )
                        if not try_again():
                            return
                    else:
...
def tags_editing(instance, full_list, is_offline=False):
    ...
    if dict_art_tags:
        print(
            ...
            sorted(dict_art_tags.items(), key=lambda x: x[1], reverse=True),
        )
        if not is_offline:
...
def check_sync_date(sync_date, length_of_current_list, ret_val, is_offline):
    ...
    if not is_offline:
        ...
        print(
            f"The last synchronization was {datetime.datetime.fromtimestamp(int(sync_date))}."
        )
        return False
...
def get_starting_side(ret_args):
    ...
    while not start_from:
        start_from = input(
            "Would you like to retrieve articles starting from the [O]ldest or [N]ewest? (Default is Newest) "
        ).lower()
...
def article_retrieval_quantity(sort_type):
    ...
    while not arts_to_retrieve:
        arts_to_retrieve = input(
            f"How many of the {sort_type} articles would you like to get? "
            "(Default is all, 0 exits the program) "
        ).lower()
...
def retrieve_articles(instance, is_offline):
    ...
    if not items_list and not get_all:
        ...
        if check_sync_date(items_list[0], len(items_list[1]), art_count, is_offline):
            ...
            art_dict = {
                k: v for i, (k, v) in enumerate(items_list[1].items()) if i < art_count
            }
    else:
        if check_sync_date(items_list[0], len(items_list[1]), art_count, is_offline):
            if ret_args["sort"] != "oldest":
...
def main():
    ...
    try:
        ...
        while cont not in ["y", "n"]:
            cont = input(
                "There has been a connection error. Would you like to continue with only the saved articles? (Y/N) "
            ).lower()
            if cont == "n":
                exit_strategy()
            elif cont != "y":
    ...
    retrieval_time, master_article_dictionary = retrieve_articles(
        pocket_instance, offline
    )

    # Option to check for and delete duplicates
    check_dupes = input(
        "Would you like to check for and delete any duplicate articles? "
        'Type "y" to check, any other character to skip. '
    ).lower()
    if check_dupes == "y":
        print()
        master_article_dictionary = del_dupes(
            master_article_dictionary, pocket_instance
        )
    ...
    if not offline:
        while True:
            choice = input(
                "What would you like to do ([A]dd/[D]elete/[V]iew/[T]ags/[E]xit)? "
            ).lower()
            if choice == "e":
                exit_strategy()
            elif choice == "a":
